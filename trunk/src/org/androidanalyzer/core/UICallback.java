...
/**
 * Callback interface used by the Analyzer Core to notify
 * the UI for progress with the analysis
 * 
 * @author k.raev
 */
public interface UICallback {
   ...
   * 
   * The Analyzer Core calls this method to notify the UI for the events of the analysis process.
   * The hashtable provided contains values with this class fields.
   * The number of all plugins are as value with key NUMBER_OF_PLUGINS.
   * The name of the plugin analyzing in the moment is set with key NAME_OF_PLUGIN.
   * If the plugin failed PLUGIN_FAILED is set/present in the table.
   * When plugin is starting its analysis PLUGIN_STARTED_ANALYZING key is present in the hashtable.
   * When plugin has finished its analysis PLUGIN_FINISHED_ANALYZING key is present in the hashtable.
   * 
   * @param props Hashtable with values for the update
   */
